...
import {
  ...
  TouchableOpacity, Modal, Image, Pressable,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { Sparkles, Flame, Droplet, Play, X, ChevronDown, ChevronUp } from "lucide-react-native";
...
type Exercise = {
  id: string; name: string; video: string; thumb: string;
...
function ExerciseRow({ ex, onPlay }: { ex: Exercise; onPlay: () => void }) {
  return (
    <Pressable style={s.exRow} onPress={onPlay} testID={`exercise-${ex.id}`}>
...
function VideoModal({ visible, exercise, onClose }: { visible: boolean; exercise: Exercise | null; onClose: () => void }) {
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose} transparent>
      <View style={s.vidBg}>
        <View style={s.vidCard}>
          <TouchableOpacity testID="video-close" style={s.vidClose} onPress={onClose}>
          ...
          <View style={s.vidInfo}>
            <Text style={s.vidName}>{exercise.name}</Text>
            <Text style={s.vidMeta}>{exercise.sets_reps}</Text>
            <Text style={s.vidTip}>{exercise.tip}</Text>
          </View>
        </View>
      </View>
    </Modal>
      ...
      {open && (
        <View style={{ marginTop: 12, gap: 10 }}>
          {day.exercises.map((ex) => (
            <ExerciseRow key={ex.id} ex={ex} onPlay={() => setActiveEx(ex)} />
          ))}
        </View>
      )}
      <VideoModal visible={!!activeEx} exercise={activeEx} onClose={() => setActiveEx(null)} />
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  exThumbWrap: { width: 90, height: 70, borderRadius: 10, overflow: "hidden", backgroundColor: "#000", position: "relative" },
  ...
  vidBg: { flex: 1, backgroundColor: "rgba(0,0,0,0.92)", justifyContent: "center", padding: 16 },
  vidCard: { backgroundColor: colors.surfaceElevated, borderRadius: 24, overflow: "hidden", borderColor: colors.border, borderWidth: 1 },
  vidClose: { position: "absolute", top: 12, right: 12, zIndex: 2, padding: 8, backgroundColor: "rgba(0,0,0,0.6)", borderRadius: 999 },
  video: { width: "100%", height: 280, backgroundColor: "#000" },
  vidInfo: { padding: 20 },
  vidName: { color: colors.text, fontSize: 22, fontWeight: "900" },
  vidMeta: { color: colors.primary, marginTop: 4, fontWeight: "700", letterSpacing: 0.5 },
  vidTip: { color: colors.textMuted, marginTop: 12, lineHeight: 20 },
});
